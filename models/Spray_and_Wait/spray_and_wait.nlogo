...
turtles-own [
  node-id ;インスタンスの番号
  msg-cnt ;生成したメッセージの数
  buffer  ;[[msg-id, src-id, dst-id, ttl, copies] ...]
  l-buffer;bufferの制限
...
to setup-messages
  ask one-of turtles [
    ...
    set buffer lput (list msg-id src-id dst-id ttl copies) buffer
    set color brown
    show (word "Message generated " src-id " to " dst-id)
    show (word "Message TTL (hops)" ttl)
    show (word "Message Copies" copies)
  ]
end
...
to forward-messages
  ...
  ask turtles [
    ...
    foreach buffer [
      ...
      let ttl item 3 msg
      let copies item 4 msg

      let sender-copies copies
          ...
          if (not member? (list msg-id ([node-id] of receiver)) ([forwarded-list] of sender)) and (empty? filter [m -> item 0 m = msg-id] ([buffer] of receiver))[
            ...
            ifelse ([node-id] of receiver) = dst-id [
              if not member? msg-id ([delivered-list] of receiver) [
                ...
                set color red
                show (word "Message arrived " src-id " to " dst-id)
              ]
              ...
              ;Spray
              if copies > 1 [
                let receiver-copies ceiling (copies / 2) ;受信側に送るコピー数（切り上げ）
                set sender-copies floor (copies / 2)   ;送信側に残すコピー数（切り捨て）

                let send-msg-sw replace-item 3 (replace-item 4 msg receiver-copies) (ttl - 1)

                set buffer lput send-msg-sw buffer

                ;送信側の転送履歴に追加
                ask sender [set forwarded-list lput (list msg-id ([node-id] of receiver)) forwarded-list]

                set color green
                show (word "Message sprayed " src-id " to " dst-id " (" receiver-copies ")")
      ...
      let updated-msg replace-item 4 msg sender-copies
      set new-buffer lput updated-msg new-buffer
